...
imageTypes = {
    ...
    "Apple DOS 3.3": "-i8a",
    "DSK": "-i9",
    "Apple DOS 400/800K": "-i10",
    "EMU": "-i11",
    "EMU2": "-i12",
    "Amiga Disk Spare": "-i13",
    "DEC RX01": "-i14",
    "DEC RX02": "-i15",
    "CBM MicroProse": "-i16",
    "CBM RapidLok": "-i17",
...
imageFmts = {
    ...
    "-i8a": "Apple DOS 3.3",
    "-i9": "DSK",
    "-i10": "Apple DOS 400/800K",
    "-i11": "EMU",
    "-i12": "EMU2",
    "-i13": "Amiga Disk Spare",
    "-i14": "DEC RX01",
    "-i15": "DEC RX02",
    "-i16": "CBM MicroProse",
    "-i17": "CBM RapidLok",
...
outputTrackOrders = {
    "Side 0 descending": "-oo1",
    "Side 1 descending": "-oo2",
    "Side 1 then side 0": "-oo4",
    "Side oriented": "-oo8",
}
outputTrackOrdersRv = {
    "-oo1": "Side 0 descending",
    "-oo8": "Side oriented",
    "-oo2": "Side 1 descending",
    "-oo4": "Side 1 then side 0",
}
...
class addTypes(QMainWindow, chooseFormatsGUI.Ui_addTypes):
    def __init__(self, parent=None):
        ...
        # set most of the combo boxes to not editable/greyed out by default
        # (that will be changed if the check box is uncheckedd
    ...
    def save_and_add(self):
        ...
        if os.stat(spath).st_size != 0:
            ...
            with open(spath, 'r', encoding="utf-8") as settings_file:
                settings = json.load(settings_file)
                # create a variable to store the current input in the image
                # type comobo box
                imgType = imageTypes[str(self.imageTypeCB.currentText())]
                # print imgType
                # create a dictionary to store the set of settings for one disk
                # (if needed)
                diskSetting = {}
                ...
                if not self.sideModeDefault.isChecked():
                    # if the side mode combo box is not set to default add
                    # the contents of it to the diskSetting dictionary
                    diskSetting.update(
                        {"Side mode": sideModes[str(self.sideModeCB.currentText())]}
                    )
                if self.flippyMode.isChecked():
                    diskSetting.update({"Flippy mode": "-y"})

                if not self.sectorSizeDefault.isChecked():
                    # if the sector size combo box is not set to default
                    # add the contents of it to the diskSetting dictionary
                    diskSetting.update(
                        {
                            "Sector size": sectorSizes[
                                str(self.sectorSizeCB.currentText())
                            ]
                        }
                    )

                if not self.sectorCountDefault.isChecked():
                    # if the sector count combo box is not set to default
                    # add the contents of it to the diskSetting dictionary
                    diskSetting.update(
                        {
                            "Sector count": sectorCounts[
                                str(self.sectorCountCB.currentText())
                            ]
                            + str(self.sectorCount.text())
                        }
                    )

                if not self.trackDistanceDefault.isChecked():
                    # if the track distance combo box is not set to default
                    # add the contents of it to the diskSetting dictionary
                    diskSetting.update(
                        {
                            "Track Distance": trackDistances[
                                str(self.trackDistanceCB.currentText())
                            ]
                        }
                    )
                ...
                if not self.targetRPMDefault.isChecked():
                    # if the target RPM combo box is not set to default
                    # add the contents of it to the diskSetting dictionary
                    diskSetting.update(
                        {
                            "Target RPM": targetRPMsBasic[
                                str(self.targetRPMCB.currentText())
                            ]
                        }
                    )
            ...
            # open the spath file and print the contents to the
            # text browser in the gui, one per format instance
            ...
            # this is a hangover from earlier versions where I
            # needed to id/process the imagetype in a unique way
            imgType = imageTypes[str(self.imageTypeCB.currentText())]
            # print imgType
            # create a dictionary to store the set of settings for
            # one disk (if needed)
            diskSetting = {}
            diskSetting.update({"Image type": imgType})
            # Check to see  if the side mode combo box is set to default
            if not self.sideModeDefault.isChecked():
                # if the side mode combo box is not set to default
                # add the contents of it to the diskSetting dictionary
                diskSetting.update(
                    {"Side mode": sideModes[str(self.sideModeCB.currentText())]}
                )
            ...
            if not self.sectorSizeDefault.isChecked():
                ...
                # add the contents of it to the diskSetting dictionary
                diskSetting.update(
                    {"Sector size": sectorSizes[str(self.sectorSizeCB.currentText())]}
                )
            if self.flippyMode.isChecked():
                diskSetting.update({"Flippy mode": "-y"})
            ...
            if not self.targetRPMDefault.isChecked():
                ...
                # add the contents of it to the diskSetting dictionary
                diskSetting.update(
                    {"Target RPM": targetRPMsBasic[str(self.targetRPMCB.currentText())]}
                )
    ...
    def updateTextBrowserFromJSON(self):
        ...
        with open(spath, encoding="utf-8") as settings_file:
            ...
            if "outputFormats" in settings:
                for out_fmt in settings["outputFormats"]:
                    num = num + 1
                    # create a variable to add content to be put into the
                    # text browser in the gui
                    setting = ""

                    for x in out_fmt:
                        # For each format add the information to the
                        # setting variable, starting with Image type
                        if x == "Image type":
                            setting = x + ": " + imageFmts[str(out_fmt[x])] + setting
                        elif x == "Sector size":
                            setting = (
                                setting
                                + ", "
                                + x
                                + ": "
                                + sectorSizesRv[str(out_fmt[x])]
                            )

                        elif x == "Side mode":
                            setting = (
                                setting + ", " + x + ": " + sideModesRv[str(out_fmt[x])]
                            )
                            ...
                            setting = (
                                ...
                                + x
                                + ": "
                                + sectorCountsRv[str(out_fmt[x])[0]]
                                + str(self.sectorCount.text())
                            )
                            ...
                            setting = (
                                ...
                                + x
                                + ": "
                                + trackDistancesRv[str(out_fmt[x])]
                            )
                            ...
                            setting = (
                                ...
                                + x
                                + ": "
                                + targetRPMsBasicRv[str(out_fmt[x])]
                            )

                        elif x == "Flippy mode":
                            setting = setting + ", " + x + ": " + str(out_fmt[x])

                        elif x == "Start track":
                            setting = setting + ", " + x + ": " + str(out_fmt[x])[2:]
    ...
    # set up functions to change the editablility of the
    # various widgets when the checkboxes change to unchecked
